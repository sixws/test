...
function drawEnemies(ctx) {
  ...
  for (const e of enemies) {
    ...
    const fill = flashing ? 'rgba(255,255,255,0.85)' : U.hexA(e.col, 0.13);
    ctx.fillStyle = fill;
    ...
    switch (e.type) {
      ...
      case 'boss': {
        ...
        ctx.fillStyle = flashing ? 'rgba(255,255,255,0.85)' : U.hexA(e.col, 0.16);
        ctx.fill();
    ...
    if ((e.type === 'tank' || e.elite) && !e.boss && e.hp < e.maxHp) {
      ...
      ctx.strokeStyle = U.hexA(e.col, 0.75);
      ctx.lineWidth = 2.4;
      ctx.beginPath();
